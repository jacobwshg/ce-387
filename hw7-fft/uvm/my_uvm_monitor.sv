...
class my_uvm_monitor_output extends uvm_monitor;
	...
	virtual function void build_phase( uvm_phase phase );
		...
		mon_ap_output = new(
			.name( "mon_ap_output" ), .parent( this )
		);
		...
		begin
			`uvm_fatal(
				"MON_OUT_BUILD", 
				$sformatf( "Failed to open real outfile %s...", OUTFILE_RE )
			);
		end
		...
		begin
			...
			`uvm_fatal(
				"MON_OUT_BUILD", 
				$sformatf( "Failed to open imag outfile %s...", OUTFILE_IM )
			);
		end
	...
	virtual function void final_phase( uvm_phase phase );
		...
		`uvm_info(
			"MON_OUT_FINAL",
			$sformatf( "Closing file %s...", OUTFILE_RE ),
			UVM_LOW
		);
		$fclose( outfile_re );
		`uvm_info(
			"MON_OUT_FINAL",
			$sformatf( "Closing file %s...", OUTFILE_IM ),
			UVM_LOW
		);
		$fclose( outfile_im );
...
class my_uvm_monitor_compare extends uvm_monitor;
	...
	virtual function void build_phase( uvm_phase phase );
		...
		begin
			`uvm_fatal(
				"MON_CMP_BUILD",
				$sformatf("Failed to open real cmpfile %s...", CMPFILE_RE )
			);
		end
		...
		begin
			...
			`uvm_fatal(
				"MON_CMP_BUILD",
				$sformatf("Failed to open imag cmpfile %s...", CMPFILE_IM )
			);
		end
