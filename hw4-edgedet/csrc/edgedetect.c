...
void write_grayscale_bmp(
	const char *filename, unsigned char* header, unsigned char* data
)
{
	...
	for ( y = 0; y < height; ++y ) 
	{
		for ( x = 0; x < width; ++x ) 
		{
			...
			*/
			++ofs;
			struct pixel *tmp_px = &data_temp[ ofs ];
			tmp_px->b = tmp_px->g = tmp_px->r = data[ ofs ];
		}
	}
