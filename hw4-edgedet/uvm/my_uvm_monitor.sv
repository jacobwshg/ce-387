...
class my_uvm_monitor_output extends uvm_monitor;
	...
	virtual function void build_phase( uvm_phase phase );
		...
		begin
			`uvm_fatal(
				"MON_OUT_BUILD",
				$sformatf( "Failed to open output file %s...", IMG_OUT_NAME )
			);
		end
	...
	virtual task run_phase( uvm_phase phase );
		...
		if (
			!uvm_config_db#( logic[ 0:BMP_HEADER_SIZE-1 ][ 7:0 ] )::get(
				null, "*", "bmp_header", bmp_header
			)
		)
		begin
			`uvm_fatal(
				"MON_OUT_RUN",
				$sformatf( "Failed to retrieve BMP header data for file %s...", IMG_CMP_NAME )
			);
		end
	...
	virtual function void final_phase( uvm_phase phase );
		...
		`uvm_info(
			"MON_OUT_FINAL",
			$sformatf( "Closing file %s...", IMG_OUT_NAME ), UVM_LOW
		);
		$fclose( out_file );
...
class my_uvm_monitor_compare extends uvm_monitor;
	...
	virtual function void build_phase( uvm_phase phase );
		...
		begin
			`uvm_fatal(
				"MON_CMP_BUILD",
				$sformatf( "Failed to open file %s...", IMG_CMP_NAME )
			);
		end

		// store the BMP header as packed array
		n_bytes = $fread( bmp_header, cmp_file, 0, BMP_HEADER_SIZE );
		uvm_config_db#( logic [ 0:BMP_HEADER_SIZE-1 ] [ 7:0 ] )::set(
			null, "*", "bmp_header",  {>> 8 { bmp_header } }
		);
	endfunction: build_phase
	...
	virtual task run_phase( uvm_phase phase );
		...
		begin
			...
			begin
				...
				begin
					n_bytes = $fread(
						pixel, cmp_file, BMP_HEADER_SIZE+i, BYTES_PER_PIXEL
					);
	...
	virtual function void final_phase( uvm_phase phase );
		super.final_phase( phase );
		`uvm_info(
			"MON_CMP_FINAL", $sformatf( "Closing file %s...", IMG_CMP_NAME ), UVM_LOW
		);
		$fclose( cmp_file );
	endfunction: final_phase
